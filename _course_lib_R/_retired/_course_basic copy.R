...
file_asym_stats = function(fileName="../_course_lib_R/data/fg_asym_pi_needles_test1.txt", cntProbeLmt,
                           xName="signifDigits", yNames=c("cntProbe", "runtime")) 
{
  ...
  for (xIdx in seq_len(length(xVals))) {
    row = xVals[xIdx]
    dfs = df[df[[xName]] == xVals[xIdx] ,]      
    sampleSizeFull = length(dfs[[xName]])      
    cat("\nxIdx =", xIdx, "xName =", xName, "\n"); print(row) ; print(dfs) ; print(sampleSizeFull)
    for (yIdx in seq_len(length(yNames))) {
  ...
  cat(sep="", ".. file_asym_stats: created a table of statistics named as = ", fileStatsPath, "\n")
